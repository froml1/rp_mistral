...
synthesis_context_block() formats the collection for prompt injection in step 6.
"""
...
_PROMPT = """\
Read this RP scene and produce a detailed narrative summary.
...
def run_synthesis(scenes_dir: Path, lore_dir: Path) -> Path:
    ...
    for scene_file in scene_files:
        ...
        new_count += 1
        n_chars = len(entry["characters"])
        n_tensions = len(entry["tensions"])
        print(f"    [lore_how] {scene_id}: {n_chars} chars, {n_tensions} tensions")

    print(f"  [lore_how] done — {new_count} new scenes, {len(scenes_data)} total")
    return lore_how_path
